...
public static class BreakoutSystems
{
        ...
        for (int i = 0; i < blockRows; i++)
        {
            for (int j = 0; j < blockCols; j++)
            {
                ...
                cmd.CreateEntity()
                    ...
                    .With(
                        new Sprite(pixel, Color.BlanchedAlmond, new Point(blockWidth, blockHeight))
                    )
    ...
    public static void BallPaddleCollision(
        Query<BallQuery> ballQuery,
        Query<PaddleQuery> paddleQuery
    )
    {
        var ball = ballQuery.Single();
        var paddle = paddleQuery.Single();

        Rectangle ballRec = new(
            ball.pos.position.ToPoint(),
            new(ball.hitbox.width, ball.hitbox.heights)
        );
        Rectangle paddleRec = new(
            paddle.pos.position.ToPoint(),
            new(paddle.hitbox.width, paddle.hitbox.heights)
        );
    ...
    [System]
    public static void BallBrickCollision(
        Query<BallQuery> ballQuery,
        Query<BlockQuery> blocksQuery,
        CommandBuffer cmd
    )
    {
        var ball = ballQuery.Single();
        ...
        {
            Rectangle blockRec = new(
                block.pos.position.ToPoint(),
                new(block.hitbox.width, block.hitbox.heights)
            );
            var collision = GetCollisionSide(ballRec, blockRec);

            if (collision != CollisionSide.None)
            {
                // Destroy the brick using the extracted entity
                cmd.DestroyEntity(block.entity);

                // Calculate the new velocity
                ball.vel.velocity = collision switch
                {
                    CollisionSide.Top or CollisionSide.Bottom => new(
                        ball.vel.velocity.X,
                        -ball.vel.velocity.Y
                    ),
                    CollisionSide.Left or CollisionSide.Right => new(
                        -ball.vel.velocity.X,
                        ball.vel.velocity.Y
                    ),
                    _ => ball.vel.velocity,
    ...
    public static void DrawSystem(Query<DrawQuery> query, Res<Graphics> graphicsRes)
    {
        ...
        foreach (var draw in query)
        {
            var rec = new Rectangle(
                draw.pos.position.ToPoint(),
                new Point(draw.hitbox.width, draw.hitbox.heights)
            );
