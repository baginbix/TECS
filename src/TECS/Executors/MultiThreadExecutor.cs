...
namespace TECS.Executors
{
    public class MultiThreadExecutor : IExecutor
    {
        ...
        public void Execute(List<SystemNode> phaseNodes, ECS engine)
        {
            ...
            foreach(var cmd in buffers) cmd.Flush(engine);
        }

        private void DispatchNode(SystemNode node, ECS engine, CommandBuffer cmd, CountdownEvent phaseBarrier)
        {
            ThreadPool.QueueUserWorkItem(_ =>
            {
                try
                {
                    node.System.Run(engine, cmd);
                }
                finally
                {
